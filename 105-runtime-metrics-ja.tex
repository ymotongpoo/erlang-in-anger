...
製品利用を考えた際、Erlang VMで最も良いセールスポイントの一つとして、ありとあらゆる内部調査やデバッグ、プロファイリング、実行時分析の透過性が挙げられます。
...
プログラム上で取得できるランタイムメトリクスがある利点として、これらメトリクスに頼ればビルドツールも簡単ですし、何らかのタスクや監視を自動化するのも同じように単純です %TODO; building tool = ビルドツールで良い？
\footnote{自動化プロセスが何かしら是正措置を行おうとして、暴走したり、やり過ぎたりしないか保証するほうが、より一層複雑です}。 %TODO: run away=暴走、go overboard=やり過ぎる、という訳で良いのかな
...
製品システムを健全に保ちつつ成長させる実用的な方法は、すべての角度から―大域的にも、局所的にも―監視できるようにすることです。
%There's no generic recipe to tell in advance what is going to be normal or not.
今起きていることが通常の挙動なのか否かを先んじて知らせる一般的な方法はありません。
...
この章(また、このあとの章のほとんど）では、紹介する概念や特徴のほとんどは、標準ライブラリ―正規OTPディストリビューションの一部―に含まれるコードからアクセス可能です。 % TODO: concept, featuresを辞書通りに訳してるけどこれで良いのだろうか。featureは特徴量的なイメージが近そうではあるが。。。
...
しかしながら、これらは一箇所にまとまっているわけではありませんし、自ら製品システムの首を締めることも非常に簡単にできてしまいます。
...
したがって、これらのテキストをより軽快にで利用しやすくするために、
共通的な操作は\otpapp{recon}\footnote{\href{http://ferd.github.io/recon/}{http://ferd.github.io/recon/}}ライブラリにまとめ直されています。 % TODO: textとは一体…
これらは一般的に、製品システムに利用しても安全です。
\section{Global View}
