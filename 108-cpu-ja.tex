...
\begin{enumerate}
  ...
  \item ロングスケジュールモニター\footnote{システムモニター(erlang:system\_monitor/2)で監視できる項目に long_schedule があります。これは NIF や driver で bump reductions をせずにCPUガメるひとを見つけるための監視項目です。\href{http://erlang.org/doc/man/erlang.html#system_monitor-2}{http://erlang.org/doc/man/erlang.html#system_monitor-2}も参考になるかと思います}が、CPU やスケジューラの使いすぎを見つけるのに便利なのはなぜですか？
