...
メモリフラグメンテーションの問題は\ref{subsec:erlang-memory-model}の章で記述されているようにErlangのメモリモデルと密接に関連しています。これは長時間動いているErlangノード(多くの場合は個々のノードの稼働時間は何ヶ月にも達する)の最も難しい問題の一つで、比較的まれに見られます。
